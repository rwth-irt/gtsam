...
typedef FastSet<FactorIndex> FactorIndexSet;
  ...
  public:
   ...
   /// get Factor Type
   uint64_t getTypeID() const {return factorTypeID_;};

   /// get Factor Name
   std::string getName() const {return factorName_;};

   void setTypeID(uint64_t typeID) {factorTypeID_ = typeID;};
   void setName(std::string name) {factorName_ = std::move(name);};
